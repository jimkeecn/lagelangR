...
const app = express();
var corsOptions = {
  origin: ["http://localhost:8081","http://localhost:4200","https://www.lglrcx.com","http://www.lglrcx.com"]
};
...
app.post("/data/:id/:id2", async (req, res, next) => { 
  try {
...
app.post("/edit/:id/:id2", async (req, res, next) => { 
  try {
    let guild_id = req.params.id;
    let log_id = req.params.id2;
    let content = req.body;
...
app.post("/delete/:id/:id2", async (req, res, next) => { 
  try {
...
app.get("/data/:id/:id2", async (req, res, next) => { 
  try {
    let guild_id = req.params.id;
    let log_id = req.params.id2;
    let path = `/${guild_id}/${log_id}`;
...
app.post("/createAccount", async (req, res, next) => { 
  try {
    ...
    if (isValid) {
      ...
      await mdb.push("/counter", data ? Object.keys(data).length + 1 : 0,false);
...
app.post("/loginWithPassword", async (req, res, next) => { 
  try {
    let content = req.body;
...
app.post("/getGuildCode", async (req, res, next) => { 
  try {
    let content = req.body;
...
function accountValidate(account) {
  if (!validator.isDate(new Date(account.birthday)) || account.password.length <= 6) {
    return false
  }
  return true;
}
